...
- **The only path that ever mints a token**: the frontend calls `Marketplace.buy()` with ETH. The Marketplace mints through `SkillCollection`, which fires a `Purchased` and a `TransferSingle` event. The server then refreshes that wallet's entitlement, and the skill becomes usable in a loadout. See [Smart Contracts](/smart-contracts) for the full sequence.
...
<Tabs>
  ...
  <Tab title="In-memory only, always">
    - Rooms and matches currently in progress. This is intentional: a match lasts minutes and is tied to a live socket connection, so there is no meaningful "resume a room" story after a server restart; every client already has to reconnect and re-authenticate from scratch.
